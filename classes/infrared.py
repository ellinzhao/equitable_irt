import os

import cv2
import numpy as np
import scipy.signal as sps
from scipy.constants import convert_temperature as conv_temp

from face_utils import load_json
from utils import load_im
from utils import raw2temp
from utils import ir2rgb
from utils import coords_ir_to_rgb


class Infrared:

    WH = (160, 120)
    BG_ROIS = [((0, 20), (0, 20)), ((WH[0] - 20, WH[0]), (0, 20))]
    LOAD_ROIS = ['forehead']

    def __init__(self, dataset_dir, name, session, duration, temp_env, units='F'):
        self.dataset_dir = dataset_dir
        self.temp_env = temp_env
        self.units = units
        self.name = name
        self.session = session
        self.duration = duration
        self.session_dir = os.path.join(dataset_dir, 'data', name, session)

        json_path = os.path.join(dataset_dir, 'landmarks', name, f'{session}.json')
        self.landmarks, self.roi_pts, self.invalid = load_json(json_path, duration)
        self.data = self.load()

        # Extract and process before aligning to RGB
        self.bg = self.extract_background()
        nuc_flag = self.find_nuc_regions()
        self.invalid = self.invalid.astype(bool) | nuc_flag.astype(bool)

        self.data = self.correct_nuc()
        self.rois = self.extract_rois()

        # Aligning IR and RGB only depends on camera homography
        self.data, self.landmarks = self.align_to_rgb()
        self.aligned = True

    def load(self):
        n = self.duration
        w, h = Infrared.WH
        ir_images = np.zeros((h, w, n))
        for i in range(n):
            ir_path = os.path.join(self.session_dir, f'ir{i}.png')
            ir = load_im(ir_path, transform=lambda a: raw2temp(a, self.units))
            ir_images[..., i] = ir
        return ir_images

    def find_nuc_regions(self):
        n = self.duration
        bg = self.bg
        # TODO: make the following code agnostic to temp units.
        peaks, _ = sps.find_peaks(bg, height=self.temp_env + 1.5, width=4 * 5)
        nuc_i = []
        for p in peaks:
            nuc_i.extend(np.arange(max(p - 4 * 5, 0), min(p + 4 * 10, n)))
        nuc_i = np.array(nuc_i, dtype=int)
        nuc_flag = np.zeros(n)
        nuc_flag[nuc_i] = 1
        return nuc_flag

    def temp_roi(self, im, bbox, agg_fn=np.mean):
        ylim, xlim = bbox
        region = im[xlim[0]:xlim[1], ylim[0]:ylim[1]]
        return agg_fn(region)

    def extract_background(self):
        temp_bgs = np.zeros(self.duration)
        thres = conv_temp(80, 'F', self.units)
        for i in range(self.duration):
            temp_bg = []
            frame = self.data[..., i]
            for roi in self.BG_ROIS:
                ylim, xlim = roi
                bg = frame[xlim[0]:xlim[1], ylim[0]:ylim[1]]
                if bg[bg < thres].size == 0:
                    temp_bg += [np.nan]
                else:
                    temp_bg += [np.nanmean(bg[bg < thres])]
            temp_bgs[i] = np.nanmean(temp_bg)
        return temp_bgs

    def extract_rois(self):
        temps = np.zeros((len(Infrared.LOAD_ROIS), self.duration))
        for i in range(self.duration):
            frame = self.data[..., i]
            rois = self.roi_pts[i]
            if len(rois) == 0:
                temps[..., i] = np.nan
            else:
                temps[..., i] = [self.temp_roi(frame, rois[loc]) for loc in Infrared.LOAD_ROIS]
        roi_dict = {}
        for j, loc in enumerate(Infrared.LOAD_ROIS):
            roi_dict[loc] = temps[j]
        return roi_dict

    def correct_nuc(self):
        return self.data + (self.temp_env - self.bg)

    def align_to_rgb(self):
        (w, h), n = Infrared.WH, self.duration
        data_warp = np.zeros((h, w, n))
        rgb_landmarks = []
        for i in range(self.duration):
            data_warp[..., i] = cv2.warpPerspective(self.data[..., i], ir2rgb, (w, h))
            rgb_landmarks += [coords_ir_to_rgb(self.landmarks[i])]
        return data_warp, rgb_landmarks
